...
def test_generate_mos6502():
    processor_path, system_path = example_pair("mos6502")
    outdir = _make_workdir("mos6502_basic_")
...
def test_mos6502_undocumented_opcode_bindings():
    processor_path, system_path = example_pair("mos6502")
    data = yaml_loader.load_processor_system(str(processor_path), str(system_path))
    opcode_by_name = {
        inst["name"]: int(inst["encoding"]["opcode"]) for inst in data["instructions"]
    }
    assert opcode_by_name["LAX_ZP_UD"] == 0xA7
    assert opcode_by_name["LAX_ABS_UD"] == 0xAF
    assert opcode_by_name["SAX_ZP_UD"] == 0x87
    assert opcode_by_name["SAX_ABS_UD"] == 0x8F
    assert opcode_by_name["DCP_ZP_UD"] == 0xC7
    assert opcode_by_name["DCP_ABS_UD"] == 0xCF
    assert opcode_by_name["ISC_ZP_UD"] == 0xE7
    assert opcode_by_name["ISC_ABS_UD"] == 0xEF
    assert opcode_by_name["SLO_ZP_UD"] == 0x07
...
def test_mos6502_unofficial_opcode_matrix_subset():
    processor_path, system_path = example_pair("mos6502")
    data = yaml_loader.load_processor_system(str(processor_path), str(system_path))
    actual = {
...
def test_mos6502_unofficial_control_and_sbc_presence():
    processor_path, system_path = example_pair("mos6502")
    data = yaml_loader.load_processor_system(str(processor_path), str(system_path))
    actual = {
        int(inst["encoding"]["opcode"])
