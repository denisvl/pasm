#ifndef PASM_OVERLAY_DRAW_H
#define PASM_OVERLAY_DRAW_H

#include <stdint.h>
#include <stdio.h>

static inline void pasm_overlay_put_pixel(
    uint32_t *pixels,
    uint32_t w,
    uint32_t h,
    int x,
...
static inline void pasm_overlay_blend_pixel(
    uint32_t *pixels,
    uint32_t w,
    uint32_t h,
    int x,
...
static inline void pasm_overlay_fill_rect(
    uint32_t *pixels,
    uint32_t w,
    uint32_t h,
    int x,
    ...
    for (int yy = 0; yy < rh; ++yy) {
        for (int xx = 0; xx < rw; ++xx) {
            pasm_overlay_put_pixel(pixels, w, h, x + xx, y + yy, color);
        }
    }
}

static inline void pasm_overlay_fill_rect_alpha(
    uint32_t *pixels,
    uint32_t w,
    uint32_t h,
    int x,
    ...
    for (int yy = 0; yy < rh; ++yy) {
        for (int xx = 0; xx < rw; ++xx) {
            pasm_overlay_blend_pixel(pixels, w, h, x + xx, y + yy, rgb, alpha);
        }
    }
}

static inline const uint8_t *pasm_overlay_glyph(char ch) {
...
static inline void pasm_overlay_draw_char(
    uint32_t *pixels,
    uint32_t w,
    uint32_t h,
    int x,
    ...
    if (scale < 1) scale = 1;
    const uint8_t *glyph = pasm_overlay_glyph(ch);
        ...
        for (int col = 0; col < 5; ++col) {
            ...
            for (int yy = 0; yy < scale; ++yy) {
                for (int xx = 0; xx < scale; ++xx) {
                    pasm_overlay_put_pixel(pixels, w, h, px + xx, py + yy, color);
                }
            }
        }
    }
...
static inline int pasm_overlay_text_width(const char *text, int scale) {
    int count = 0;
...
static inline void pasm_overlay_draw_text(
    uint32_t *pixels,
    uint32_t w,
    uint32_t h,
    int x,
    ...
    for (int i = 0; text[i] != '\0'; ++i) {
        pasm_overlay_draw_char(pixels, w, h, cursor, y, text[i], scale, color);
        cursor += advance;
    }
}

static inline void pasm_overlay_draw_perf(
    uint32_t *pixels,
    uint32_t w,
    uint32_t h,
    uint32_t fps_x100,
    ...
    box_w = pasm_overlay_text_width(line, scale) + pad * 2;
    if (box_w > (int)w) {
        snprintf(
            line,
            sizeof(line),
            "F%u.%02u C%llu.%02llu %u.%u%%",
            fps_x100 / 100u,
            fps_x100 % 100u,
            (unsigned long long)(cpu_hz / 1000000u),
            (unsigned long long)((cpu_hz % 1000000u) / 10000u),
            cpu_pct_x10 / 10u,
            cpu_pct_x10 % 10u
        );
        box_w = pasm_overlay_text_width(line, scale) + pad * 2;
    ...
    pasm_overlay_fill_rect_alpha(pixels, w, h, box_x, box_y, box_w, box_h, bg_rgb, bg_alpha);
    pasm_overlay_draw_text(pixels, w, h, text_x + scale, text_y + scale, line, scale, shadow);
    pasm_overlay_draw_text(pixels, w, h, text_x, text_y, line, scale, fg);
}

static inline void pasm_overlay_update_perf(
    uint32_t tick_ms,
    ...
    uint64_t measured_cpu_hz = (dt_ms > 0u)
        ? ((cycle_delta * 1000u + (uint64_t)(dt_ms / 2u)) / (uint64_t)dt_ms)
        : 0u;
    uint32_t measured_cpu_pct_x10 = (target_cpu_hz > 0u)
        ? (uint32_t)((measured_cpu_hz * 1000u + (target_cpu_hz / 2u)) / target_cpu_hz)
        : 0u;
    ...
    *cpu_hz = (*cpu_hz == 0u)
        ? measured_cpu_hz
        : ((*cpu_hz * 3u) + measured_cpu_hz + 2u) / 4u;
    *cpu_pct_x10 = (*cpu_pct_x10 == 0u)
        ? measured_cpu_pct_x10
        : ((*cpu_pct_x10 * 3u) + measured_cpu_pct_x10 + 2u) / 4u;
