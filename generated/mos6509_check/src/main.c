...
int main(int argc, char *argv[]) {
    CPUState *cpu = mos6509_create(1048576);
    if (!cpu) {
    ...
    if (system_dir) {
        if (mos6509_load_system_roms(cpu, system_dir) != 0) {
    ...
    if (rom_file) {
        if (mos6509_load_rom(cpu, rom_file, load_addr) != 0) {
    ...
    if (test_name) {
        ...
        if (max_cycles > 0) {
            mos6509_run_until(cpu, max_cycles);
            printf("Executed %llu cycles\n", cpu->total_cycles);
        } else {
            mos6509_run(cpu);
        }
