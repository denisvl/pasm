...
static void cpu_components_step_post(CPUState *cpu, DecodedInstruction *inst, uint16_t pc_before) {
    ...
    }
    {
        ...
        }
        {
            ...
            int key_count = 0;
            const uint8_t *ks = cpu_host_hal_keyboard_state(&key_count);
            /* Prefer controller-map when loaded (host_controller_sg1000.yaml); otherwise keep legacy keyboard-map behavior. */
            if (g_runtime_controller_map.loaded != 0u) {
                ...
                if (cpu_component_controller_pressed("P2_B2"))   comp->joy2 &= (uint8_t)~0x20u;
                {
                    ...
                    /* Preserve legacy env override behavior (PASM_SG1000_JOY2_CONNECTED=0 disables port 2 even if map says connected). */
...
uint8_t z80_read_byte(CPUState *cpu, uint16_t addr) {
    {
        ...
        if (addr < 0xC000u) {
            ...
            /*
             * SG-1000 "no mapper" cartridges are often 8K/16K/32K and are
             * mirrored through the 0x0000-0xBFFF window by the bus wiring.
             * Use power-of-two masking when possible, fallback to modulo.
             */
            {
