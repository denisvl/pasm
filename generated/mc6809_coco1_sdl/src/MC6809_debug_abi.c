    ...
    for (int i = 0; i < cpu->num_break_points; i++) {
        if (cpu->break_points[i] == cpu->pc) {
            *out_addr = cpu->break_points[i];
            mc6809_clear_breakpoint(cpu, *out_addr);
            return true;
        }
    }
    return false;
...
static void dbg_restore_suppressed_breakpoint(CPUState *cpu, uint16_t addr) {
    ...
    if (!dbg_has_breakpoint(cpu, (uint64_t)addr)) {
        mc6809_set_breakpoint(cpu, addr);
    }
}
...
static uint32_t dbg_read_u32(CPUState *cpu, uint16_t addr) {
    ...
    for (size_t i = 0; i < 4u; i++) {
        ...
        uint8_t v = mc6809_read_byte(cpu, fetch_addr);
...
static void dbg_fill_disasm_row(CPUState *cpu, PASMDebugDisasmRow *row, uint16_t addr) {
    ...
    dbg_copy(row->instruction, sizeof(row->instruction), mc6809_disassemble_instruction(addr, raw));
...
static void dbg_fill_history_row(CPUState *cpu, PASMDebugHistoryRow *row, uint64_t address, uint32_t raw) {
    ...
    dbg_copy(
        ...
        mc6809_disassemble_instruction((uint16_t)(address & 0xFFFFu), raw)
    );
}

int mc6809_dbg_snapshot_counts(CPUState *cpu, PASMDebugCounts *out_counts) {
...
int mc6809_dbg_snapshot_fill(
    ...
    dbg_copy(out_core->target_name, sizeof(out_core->target_name), "MC6809");
    out_core->mode = dbg_mode(cpu);
    out_core->architecture = PASM_ARCH_MC6809;
    ...
    if (reg_rows && reg_cap > 0) {
        ...
        if (row < reg_cap) {
            PASMDebugRegisterRow *r = &reg_rows[row];
            dbg_copy(r->name, sizeof(r->name), "DP");
            {
                uint64_t val = (uint64_t)(cpu->registers[REG_DP]);
                snprintf(r->hex_value, sizeof(r->hex_value), "0x%02llX", (unsigned long long)val);
                snprintf(r->dec_value, sizeof(r->dec_value), "%llu", (unsigned long long)val);
                r->has_dec = 1u;
                r->changed = 0u;
        ...
        if (row < reg_cap) {
            PASMDebugRegisterRow *r = &reg_rows[row];
            dbg_copy(r->name, sizeof(r->name), "U");
            {
                uint64_t val = (uint64_t)(cpu->u);
                snprintf(r->hex_value, sizeof(r->hex_value), "0x%04llX", (unsigned long long)val);
                snprintf(r->dec_value, sizeof(r->dec_value), "%llu", (unsigned long long)val);
                r->has_dec = 1u;
                r->changed = 0u;
        ...
        if (row < reg_cap) {
            PASMDebugRegisterRow *r = &reg_rows[row];
            dbg_copy(r->name, sizeof(r->name), "S");
            {
    ...
    if (flag_rows && flag_cap > 0) {
        ...
        if (row < flag_cap) {
            PASMDebugFlagRow *f = &flag_rows[row];
            dbg_copy(f->name, sizeof(f->name), "F");
            f->value = cpu->flags.F ? 1u : 0u;
            f->changed = 0u;
        }
        row++;
        if (row < flag_cap) {
            PASMDebugFlagRow *f = &flag_rows[row];
            dbg_copy(f->name, sizeof(f->name), "E");
            f->value = cpu->flags.E ? 1u : 0u;
            f->changed = 0u;
        }
        row++;
    ...
    if (stack_rows && stack_cap > 0) {
        for (i = 0; i < stack_cap && i < 32u; i++) {
            ...
            row->value = (uint64_t)mc6809_read_word(cpu, (uint16_t)row->address);
    ...
    if (mem_rows && mem_cap > 0) {
        ...
        for (i = 0; i < mem_cap && i < 16u; i++) {
            ...
            for (b = 0; b < 16u; b++) {
                ...
                uint8_t v = mc6809_read_byte(cpu, mem_addr);
...
int mc6809_dbg_run(CPUState *cpu) {
    if (!cpu) return -1;
    cpu->running = true;
    return 0;
}

int mc6809_dbg_run_slice(CPUState *cpu, uint32_t max_steps, uint8_t *out_mode) {
    uint32_t i;
    int rc = 0;
...
int mc6809_dbg_run_for_cycles(CPUState *cpu, uint64_t max_cycles, uint8_t *out_mode) {
    int rc = 0;
    uint16_t suppressed_bp = 0u;
    bool had_suppressed_bp = false;
...
int mc6809_dbg_pause(CPUState *cpu) {
    if (!cpu) return -1;
    cpu->running = false;
    return 0;
}

int mc6809_dbg_step_into(CPUState *cpu) {
    if (!cpu) return -1;
    bool was_running = cpu->running;
    uint8_t mode = PASM_DBG_ERROR;
    int rc;
    cpu->running = true;
    rc = mc6809_dbg_run_slice(cpu, 1u, &mode);
    if (!was_running) {
        cpu->running = false;
    }
    return rc;
...
int mc6809_dbg_step_over(CPUState *cpu) {
    if (!cpu) return -1;
    bool was_running = cpu->running;
    int rc = 0;
    uint16_t suppressed_bp = 0u;
    ...
    bool call_like = false;
    {
        ...
        DecodedInstruction inst = mc6809_decode(decode_raw, prefix, start_pc);
...
int mc6809_dbg_step_out(CPUState *cpu) {
    if (!cpu) return -1;
    bool was_running = cpu->running;
    int rc = 0;
    uint16_t suppressed_bp = 0u;
...
int mc6809_dbg_toggle_breakpoint(CPUState *cpu, uint64_t address) {
    int i;
    uint16_t addr16;
    if (!cpu) return -1;
    ...
    for (i = 0; i < cpu->num_break_points; i++) {
        if (cpu->break_points[i] == addr16) {
            mc6809_clear_breakpoint(cpu, addr16);
            return 0;
        }
    }
    mc6809_set_breakpoint(cpu, addr16);
    return 0;
}

int mc6809_dbg_select_thread(CPUState *cpu, uint32_t thread_id) {
    (void)cpu;
    return (thread_id == 0u) ? 0 : -1;
}

int mc6809_dbg_jump_frame(CPUState *cpu, size_t frame_index) {
    (void)cpu;
    return (frame_index == 0u) ? 0 : -1;
}

int mc6809_dbg_read_memory(CPUState *cpu, uint64_t address, uint8_t *out, size_t size) {
    size_t i;
    if (!cpu || !out) return -1;
    ...
    for (i = 0; i < size; i++) {
        ...
        out[i] = mc6809_read_byte(cpu, mem_addr);
    }
    return 0;
}

int mc6809_dbg_clear_memory(CPUState *cpu) {
...
int mc6809_dbg_clear_history(CPUState *cpu) {
    if (!cpu) return -1;
    dbg_history_clear(cpu);
    return 0;
}

int mc6809_dbg_set_pc(CPUState *cpu, uint64_t address) {
    if (!cpu) return -1;
...
int mc6809_dbg_set_overlay_enabled(CPUState *cpu, uint8_t enabled) {
    if (!cpu) return -1;
    cpu->debug_overlay_enabled = (enabled != 0u);
    return 0;
}

int mc6809_dbg_get_overlay_enabled(CPUState *cpu, uint8_t *out_enabled) {
    if (!cpu || !out_enabled) return -1;
    *out_enabled = cpu->debug_overlay_enabled ? 1u : 0u;
    return 0;
}

int mc6809_dbg_focus_host_window(CPUState *cpu) {
    if (!cpu) return -1;
...
CPUState *pasm_dbg_create(size_t memory_size) {
    return mc6809_create(memory_size);
}
...
int pasm_dbg_load_rom(CPUState *cpu, const char *filename, uint16_t address) {
    return mc6809_load_rom(cpu, filename, address);
}

int pasm_dbg_load_system_roms(CPUState *cpu, const char *system_base_dir) {
    return mc6809_load_system_roms(cpu, system_base_dir);
}

int pasm_dbg_load_cartridge_rom(CPUState *cpu, const char *path) {
    return mc6809_load_cartridge_rom(cpu, path);
}
...
int pasm_dbg_snapshot_fill(
    ...
    return mc6809_dbg_snapshot_fill(
        cpu,
        out_core,
...
const char *pasm_dbg_system_name(void) {
    return "MC6809CoCo1InteractiveSystem";
}

uint8_t pasm_dbg_architecture(void) {
    return (uint8_t)PASM_ARCH_MC6809;
}
