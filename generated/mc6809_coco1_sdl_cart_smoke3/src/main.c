...
int main(int argc, char *argv[]) {
    ...
    const char *cart_rom_file = "D:\\Projects\\pasm\\examples\\roms\\Dungeons of Daggorath (1982) (26-3093) (DynaMicro) [!].ccc";
    ...
    if (system_dir) {
        if (mc6809_load_system_roms(cpu, system_dir) != 0) {
    ...
    if (test_name) {
        ...
        if (max_cycles > 0) {
            mc6809_run_until(cpu, max_cycles);
            printf("Executed %llu cycles\n", cpu->total_cycles);
        } else {
            mc6809_run(cpu);
        }
