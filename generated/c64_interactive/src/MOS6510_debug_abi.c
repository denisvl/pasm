    ...
    for (int i = 0; i < cpu->num_break_points; i++) {
        if (cpu->break_points[i] == cpu->pc) {
            *out_addr = cpu->break_points[i];
            mos6510_clear_breakpoint(cpu, *out_addr);
            return true;
        }
    }
    return false;
...
static void dbg_restore_suppressed_breakpoint(CPUState *cpu, uint16_t addr) {
    ...
    if (!dbg_has_breakpoint(cpu, (uint64_t)addr)) {
        mos6510_set_breakpoint(cpu, addr);
    }
}
...
static uint32_t dbg_read_u32(CPUState *cpu, uint16_t addr) {
    ...
    for (size_t i = 0; i < 4u; i++) {
        ...
        uint8_t v = mos6510_read_byte(cpu, fetch_addr);
...
static uint8_t dbg_instruction_len(CPUState *cpu, uint16_t addr) {
    ...
    DecodedInstruction inst = mos6510_decode(decode_raw, prefix, addr);
...
static void dbg_fill_disasm_row(CPUState *cpu, PASMDebugDisasmRow *row, uint16_t addr) {
    ...
    dbg_copy(row->instruction, sizeof(row->instruction), mos6510_disassemble_instruction(addr, raw));
...
static void dbg_fill_history_row(CPUState *cpu, PASMDebugHistoryRow *row, uint64_t address, uint32_t raw) {
    ...
    dbg_copy(
        ...
        mos6510_disassemble_instruction((uint16_t)(address & 0xFFFFu), raw)
    );
}

int mos6510_dbg_snapshot_counts(CPUState *cpu, PASMDebugCounts *out_counts) {
...
int mos6510_dbg_snapshot_fill(
    ...
    dbg_copy(out_core->target_name, sizeof(out_core->target_name), "MOS6510");
    out_core->mode = dbg_mode(cpu);
    out_core->architecture = PASM_ARCH_MOS6510;
    ...
    if (reg_rows && reg_cap > 0) {
        ...
        if (row < reg_cap) {
            PASMDebugRegisterRow *r = &reg_rows[row];
            dbg_copy(r->name, sizeof(r->name), "DDR");
            {
                uint64_t val = (uint64_t)(cpu->registers[REG_IO_DDR]);
                snprintf(r->hex_value, sizeof(r->hex_value), "0x%02llX", (unsigned long long)val);
                snprintf(r->dec_value, sizeof(r->dec_value), "%llu", (unsigned long long)val);
                r->has_dec = 1u;
                r->changed = 0u;
        ...
        if (row < reg_cap) {
            PASMDebugRegisterRow *r = &reg_rows[row];
            dbg_copy(r->name, sizeof(r->name), "PORT");
            {
                uint64_t val = (uint64_t)(cpu->registers[REG_IO_DATA]);
                snprintf(r->hex_value, sizeof(r->hex_value), "0x%02llX", (unsigned long long)val);
                snprintf(r->dec_value, sizeof(r->dec_value), "%llu", (unsigned long long)val);
                r->has_dec = 1u;
                r->changed = 0u;
    ...
    if (mem_rows && mem_cap > 0) {
        ...
        for (i = 0; i < mem_cap && i < 16u; i++) {
            ...
            for (b = 0; b < 16u; b++) {
                ...
                uint8_t v = mos6510_read_byte(cpu, mem_addr);
...
int mos6510_dbg_run(CPUState *cpu) {
    if (!cpu) return -1;
    cpu->running = true;
    return 0;
}

int mos6510_dbg_run_slice(CPUState *cpu, uint32_t max_steps, uint8_t *out_mode) {
    uint32_t i;
    int rc = 0;
...
int mos6510_dbg_run_for_cycles(CPUState *cpu, uint64_t max_cycles, uint8_t *out_mode) {
    int rc = 0;
    uint16_t suppressed_bp = 0u;
    bool had_suppressed_bp = false;
...
int mos6510_dbg_pause(CPUState *cpu) {
    if (!cpu) return -1;
    cpu->running = false;
    return 0;
}

int mos6510_dbg_step_into(CPUState *cpu) {
    if (!cpu) return -1;
    bool was_running = cpu->running;
    uint8_t mode = PASM_DBG_ERROR;
    int rc;
    cpu->running = true;
    rc = mos6510_dbg_run_slice(cpu, 1u, &mode);
    if (!was_running) {
        cpu->running = false;
    }
    return rc;
...
int mos6510_dbg_step_over(CPUState *cpu) {
    if (!cpu) return -1;
    bool was_running = cpu->running;
    int rc = 0;
    uint16_t suppressed_bp = 0u;
    ...
    bool call_like = false;
    {
        ...
        DecodedInstruction inst = mos6510_decode(decode_raw, prefix, start_pc);
...
int mos6510_dbg_step_out(CPUState *cpu) {
    if (!cpu) return -1;
    bool was_running = cpu->running;
    int rc = 0;
    uint16_t suppressed_bp = 0u;
...
int mos6510_dbg_toggle_breakpoint(CPUState *cpu, uint64_t address) {
    int i;
    uint16_t addr16;
    if (!cpu) return -1;
    ...
    for (i = 0; i < cpu->num_break_points; i++) {
        if (cpu->break_points[i] == addr16) {
            mos6510_clear_breakpoint(cpu, addr16);
            return 0;
        }
    }
    mos6510_set_breakpoint(cpu, addr16);
    return 0;
}

int mos6510_dbg_select_thread(CPUState *cpu, uint32_t thread_id) {
    (void)cpu;
    return (thread_id == 0u) ? 0 : -1;
}

int mos6510_dbg_jump_frame(CPUState *cpu, size_t frame_index) {
    (void)cpu;
    return (frame_index == 0u) ? 0 : -1;
}

int mos6510_dbg_read_memory(CPUState *cpu, uint64_t address, uint8_t *out, size_t size) {
    size_t i;
    if (!cpu || !out) return -1;
    ...
    for (i = 0; i < size; i++) {
        ...
        out[i] = mos6510_read_byte(cpu, mem_addr);
    }
    return 0;
}

int mos6510_dbg_clear_memory(CPUState *cpu) {
...
int mos6510_dbg_clear_history(CPUState *cpu) {
    if (!cpu) return -1;
    dbg_history_clear(cpu);
    return 0;
}

int mos6510_dbg_set_pc(CPUState *cpu, uint64_t address) {
    if (!cpu) return -1;
...
int mos6510_dbg_set_overlay_enabled(CPUState *cpu, uint8_t enabled) {
    if (!cpu) return -1;
    cpu->debug_overlay_enabled = (enabled != 0u);
    return 0;
}

int mos6510_dbg_get_overlay_enabled(CPUState *cpu, uint8_t *out_enabled) {
    if (!cpu || !out_enabled) return -1;
    *out_enabled = cpu->debug_overlay_enabled ? 1u : 0u;
    return 0;
}

int mos6510_dbg_focus_host_window(CPUState *cpu) {
    if (!cpu) return -1;
...
CPUState *pasm_dbg_create(size_t memory_size) {
    return mos6510_create(memory_size);
}
...
int pasm_dbg_load_rom(CPUState *cpu, const char *filename, uint16_t address) {
    return mos6510_load_rom(cpu, filename, address);
}

int pasm_dbg_load_system_roms(CPUState *cpu, const char *system_base_dir) {
    return mos6510_load_system_roms(cpu, system_base_dir);
}
...
int pasm_dbg_snapshot_fill(
    ...
    return mos6510_dbg_snapshot_fill(
        cpu,
        out_core,
...
uint8_t pasm_dbg_architecture(void) {
    return (uint8_t)PASM_ARCH_MOS6510;
}
