...
def _qt_event_host_candidates(event: QKeyEvent) -> List[str]:
    out: List[str] = []
    native_sc = int(event.nativeScanCode() or 0)
    if native_sc > 0:
        sdl_candidates: List[int] = []
        if sys.platform.startswith("linux"):
            if native_sc >= 9:
                sdl_candidates.append(native_sc - 8)
            sdl_candidates.append(native_sc)
        else:
            sdl_candidates.append(native_sc)
        for sc in sdl_candidates:
            tok = _sdl_scancode_to_host_token(sc)
    ...
    for cand in text_map.get(text, []):
        if cand and cand not in out:
            out.append(cand)
    symbolic = _qt_key_to_host_key(event)
    if symbolic and symbolic not in out:
        out.append(symbolic)
    return out
...
class MainWindow(QMainWindow):
    def __init__(self, model: KeymapperModel):
        ...
        self.bindings_list.currentTextChanged.connect(self._on_binding_selected)
        self.alias_bindings_list = QListWidget()
        self.alias_bindings_list.currentTextChanged.connect(self._on_alias_binding_selected)
        ...
        self.row_spin = QSpinBox(); self.row_spin.setRange(0, 31)
        self.bit_spin = QSpinBox(); self.bit_spin.setRange(0, 7)
    ...
    def _show_keyboard_event_debug(self, event: QKeyEvent, kind: str):
        ...
        host_key = _qt_key_to_host_key(event) or "-"
        ...
        self.statusBar().showMessage(
            f"KB {kind} host={host_key} scancode={sdl_sc_text} qt={qt_key} vk={vk} mods={mods} nmods=0x{nmods:x} text={txt_disp}"
        )
    ...
    def _update_window_title(self):
        self.setWindowTitle(f"PASM Keyboard Mapper (New) - {self._system_name()}")
    ...
    def _on_binding_selected(self, text: str):
        if not text:
            ...
            return
        host_key = text.split(" -> ", 1)[0].strip()
        self._select_host_binding(host_key)

    def _on_alias_binding_selected(self, text: str):
        if not text:
            return
        host_key = text.split(" -> ", 1)[0].strip()
    ...
    def _select_binding_item(self, host_key: str):
        ...
        for i in range(self.bindings_list.count()):
            ...
            if self._norm_host_id(lhs) == needle:
    ...
    def _refresh_alias_bindings(self):
        ...
        for b in binds:
            ...
            eid = str(b.get("emulator_key_id", "")).strip()
            sid = str(b.get("system_key_id", "")).strip()
            if "presses" in b and isinstance(b["presses"], list) and b["presses"]:
                p = b["presses"][0]
                detail = f"r{int(p.get('row', 0))}/b{int(p.get('bit', 0))}"
            elif any(k in b for k in ("ascii", "ascii_shift", "ascii_ctrl")):
                detail = "ascii"
            elif mid:
                detail = "mapper target (no payload)"
            elif eid:
                detail = f"emulator:{eid}"
            elif sid:
                detail = f"system:{sid}"
            else:
                detail = "unmapped"
            self.alias_bindings_list.addItem(f"{hk} -> {self.selected_mapper_key_id} [{detail}]")
    ...
    def refresh_bindings(self):
        ...
        for b in self.model.host_bindings():
            ...
            if sid:
                ...
                detail = f"{detail},{vf}"
            self.bindings_list.addItem(f"{hk} -> {target} [{detail}]")
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PASM keyboard mapper UI (new track)")
