...
rem One-shot helper: generate + build + run PASM Rust debugger for Atari 800XE.
rem
rem Usage:
rem   scripts\run_atari800xe_debugger.bat [interactive^|default]
rem
rem Optional env overrides:
rem   START_PC=0xE477    (optional; if unset, uses reset vector)
rem   MEMORY_SIZE=65536
rem   OUTPUT_DIR=generated/atari800xe_interactive
...
rem ROM paths (800XE reuses 800XL OS ROMs; BASIC is copied to 800XE dir)
if not defined OS_ROM_LOW set "OS_ROM_LOW=../../roms/atari800xe/ATARIXL_C000.ROM"
if not defined OS_ROM_HIGH set "OS_ROM_HIGH=../../roms/atari800xe/ATARIXL_D800.ROM"
if not defined SELFTEST_ROM set "SELFTEST_ROM=../../roms/atari800xe/ATARIXL_SELFTEST.ROM"
if not defined BASIC_ROM set "BASIC_ROM=../../roms/atari800xe/AtariBasic.rom"
...
set "DEVICE_CASSETTE_ADAPTER=examples/devices/atari800xl/atari800xl_cassette_adapter.yaml"
...
set "SYSTEM_DIR=examples/systems/atari800xe"

if /I "%PROFILE%"=="default" (
  set "SYSTEM=examples/systems/atari800xe/atari800xe_default.yaml"
  set "HOST=examples/hosts/atari800xl/atari800xl_host_stub.yaml"
  set "DEFAULT_OUTPUT=generated/atari800xe_default"
) else if /I "%PROFILE%"=="interactive" (
  set "SYSTEM=examples/systems/atari800xe/atari800xe_interactive.yaml"
  set "HOST=examples/hosts/atari800xl/atari800xl_host_hal_interactive.yaml"
  set "DEFAULT_OUTPUT=generated/atari800xe_interactive"
) else (
...
set "TMP_SYSTEM=%SYSTEM_DIR_ABS%\.tmp_atari800xe_system_%RANDOM%%RANDOM%.yaml"
uv run python -c "import yaml,sys; src,dst,osl,osh,st,bas=sys.argv[1:7]; data=yaml.safe_load(open(src,'r',encoding='utf-8')); imgs=((data.get('memory') or {}).get('rom_images') or []); [rom.__setitem__('file', bas if str(rom.get('name',''))=='atari800xe_basic' else st if str(rom.get('name',''))=='atari800xe_selftest' else osl if str(rom.get('name',''))=='atari800xe_os_low' else osh if str(rom.get('name',''))=='atari800xe_os_high' else rom.get('file')) for rom in imgs]; yaml.safe_dump(data,open(dst,'w',encoding='utf-8'),sort_keys=False)" "%SYSTEM%" "%TMP_SYSTEM%" "%OS_ROM_LOW%" "%OS_ROM_HIGH%" "%SELFTEST_ROM%" "%BASIC_ROM%"
if errorlevel 1 exit /b %errorlevel%
