...
# One-shot helper: generate + build + run PASM Rust debugger for Commodore SX-64.
#
# Usage:
#   scripts/run_csx64_debugger.sh [interactive|default]
#
# Optional env overrides:
#   START_PC=0xFCE2
#   MEMORY_SIZE=65536
#   OUTPUT_DIR=generated/csx64_interactive
#   EXTRA_CARGO_ARGS="--release"
#   RUN_SPEED=realtime|max
#   CARTRIDGE_MAP=examples/cartridges/c64/c64_cart_auto.yaml
#   CARTRIDGE_ROM_GEN=../../roms/csx64/basic.901226-01.bin
#   CARTRIDGE_ROM_RUNTIME=/abs/path/to/cart.bin
#   CARTRIDGE_DIR=/abs/path/to/csx64/roms
#   BOOT_CARTRIDGE=0|1
...
CARTRIDGE_ROM_GEN="${CARTRIDGE_ROM_GEN:-../../roms/csx64/basic.901226-01.bin}"
...
if [[ -z "${CARTRIDGE_DIR}" ]]; then
  CARTRIDGE_DIR="${REPO_ROOT}/examples/roms/csx64"
fi
...
case "${PROFILE}" in
  default)
    SYSTEM="examples/systems/csx64/csx64_cartridge_default.yaml"
    DEFAULT_OUTPUT="generated/csx64"
    ;;
  interactive)
    SYSTEM="examples/systems/csx64/csx64_cartridge_interactive.yaml"
    DEFAULT_OUTPUT="generated/csx64_interactive"
    ;;
  *)
