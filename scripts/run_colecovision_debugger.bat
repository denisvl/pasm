...
set "OUT_DIR=generated\z80_colecovision_sdl"
...
set "PASM_COLECOVISION_JOY2_CONNECTED=%PASM_COLECOVISION_JOY2_CONNECTED%"
if "%PASM_COLECOVISION_JOY2_CONNECTED%"=="" set "PASM_COLECOVISION_JOY2_CONNECTED=0"

set "SYSTEM_DIR=examples/systems"
set "CARTRIDGE_MAP=examples/cartridges/colecovision/colecovision_mapper_none.yaml"
set "CARTRIDGE_ROM_GEN=../../roms/colecovision/ColecoVision BIOS (1982).col"
set "CARTRIDGE_ROM_RUN=%PASM_COLECOVISION_ROM%"
if "%CARTRIDGE_ROM_RUN%"=="" set "CARTRIDGE_ROM_RUN=%ROOT_DIR%\examples\roms\colecovision\ColecoVision BIOS (1982).col"
set "CARTRIDGE_DIR=%ROOT_DIR%\examples\roms\colecovision"

set "HOST_YAML=examples/hosts/colecovision/colecovision_host_hal_interactive.yaml"

if /I "%PROFILE%"=="default" (
  set "OUT_DIR=generated\z80_colecovision"
  set "HOST_YAML=examples/hosts/colecovision/colecovision_host_stub.yaml"
)
set "BUILD_DIR=%OUT_DIR%\build"
...
uv run python -m src.main generate ^
  ...
  --system examples/systems/colecovision/colecovision_%PROFILE%.yaml ^
  --ic examples/ics/colecovision/colecovision_vdp_tms9928a.yaml ^
  --ic examples/ics/colecovision/colecovision_joypad_io.yaml ^
  --ic examples/ics/colecovision/colecovision_cpu_bus.yaml ^
  --ic examples/ics/colecovision/colecovision_main_ram.yaml ^
  --ic examples/ics/colecovision/colecovision_psg_sn76489.yaml ^
...
if exist "%CARTRIDGE_DIR%" (
  cargo run --manifest-path tools/debugger_tui/Cargo.toml --features linked-emulator -- ^
    ...
    --cartridge-map "%CARTRIDGE_MAP%" ^
    --cart-rom "%CARTRIDGE_ROM_RUN%" ^
    --controller-map "examples/hosts/colecovision/host_controller_colecovision.yaml" ^
    --keyboard-map "examples/hosts/colecovision/host_console_colecovision.yaml" ^
    --cartridge-dir "%CARTRIDGE_DIR%"
) else (
  echo Warning: cartridge directory not found: %CARTRIDGE_DIR%
  cargo run --manifest-path tools/debugger_tui/Cargo.toml --features linked-emulator -- ^
