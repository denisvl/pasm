...
# One-shot helper: generate + build + run PASM Rust debugger for BBC Micro Model A.
#
# Usage:
#   scripts/run_bbc_micro_model_a_debugger.sh [interactive|default]
#
# Optional env overrides:
#   START_PC=0xFFFC
#   MEMORY_SIZE=65536
#   OUTPUT_DIR=generated/bbc_micro_model_a_interactive
...
DEVICE_MONITOR="examples/devices/common/tv_crt_mono.yaml"
HOST_INTERACTIVE="examples/hosts/bbcmicro/bbc_micro_model_a_host_hal_interactive.yaml"
HOST_STUB="examples/hosts/bbcmicro/bbc_micro_model_a_host_stub.yaml"
HOST_FILE="${HOST_FILE:-}"

case "${PROFILE}" in
  default)
    SYSTEM="examples/systems/bbcmicro/bbc_micro_model_a_default.yaml"
    DEFAULT_OUTPUT="generated/bbc_micro_model_a_default"
    if [[ -z "${HOST_FILE}" ]]; then
      HOST_FILE="${HOST_STUB}"
    fi
    ;;
  interactive)
    SYSTEM="examples/systems/bbcmicro/bbc_micro_model_a_interactive.yaml"
    DEFAULT_OUTPUT="generated/bbc_micro_model_a_interactive"
    if [[ -z "${HOST_FILE}" ]]; then
