...
rem One-shot helper: generate + build + run PASM Rust debugger for Apple II Plus.
rem
rem Usage:
rem   scripts\run_apple2plus_debugger.bat [interactive^|default]
rem
rem Optional env overrides:
rem   START_PC=0xFA62
rem   MEMORY_SIZE=65536
rem   OUTPUT_DIR=generated/apple2plus_interactive
...
set "SYSTEM_DIR=examples/systems/apple2plus"
...
if /I "%PROFILE%"=="default" (
  set "SYSTEM=examples/systems/apple2plus/apple2plus_default.yaml"
  set "DEFAULT_OUTPUT=generated/apple2plus"
) else if /I "%PROFILE%"=="interactive" (
  set "SYSTEM=examples/systems/apple2plus/apple2plus_interactive.yaml"
  set "DEFAULT_OUTPUT=generated/apple2plus_interactive"
) else (
