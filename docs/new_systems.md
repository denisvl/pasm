...
- [x] **Dragon 64** ✅ (2026-08-01)
  - *Strategy:* Uses your Dragon 32 core; double the ROM to 32 KB (two 16 KB banks) for 64K native mode support.
  - *Implementation:* Created 2 system YAMLs in `examples/systems/dragon64/` (default + interactive). Reuses ALL coco1 ICs directly (SAM 6883, PIA0/PIA1 6821, VDG 6847, cart expansion, main RAM), coco1 devices (keyboard, gameport, video, speaker, cassette_transport, tv), and `host_coco` (stub + HAL interactive). 32 KB RAM. Two 16 KB ROMs: `d64_1.rom` at $8000-$BFFF (32K compatibility mode BASIC) and `d64_2.rom` at $C000-$FFFF (64K native mode BASIC). DragonDOS cartridge ROM (`ddos10.rom`, 8 KB) available via cartridge slot. Clock: 894886 Hz (0.89 MHz). Profiles: `dragon64_default` (output `generated/mc6809_dragon64`) and `dragon64_interactive` (output `generated/mc6809_dragon64_sdl`). Scripts: `run_dragon64_debugger.bat/.sh`, `run_dragon64_interactive.bat/.sh`. Codegen, build (MSVC Release), and runtime all pass — default runs `mc6809_test.exe`, interactive launches the GLFW window via the Rust debugger TUI.
