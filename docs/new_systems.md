...
- [x] **BBC Micro Model A** ✅ (2026-07-25)
  ...
  - *Known limitation (tracked in `TODO`):* The MOS still reports "32K" at boot instead of "16K" because the generated emulator uses a flat 64K `cpu->memory[]` array (ROMs load into it, so `memory_size` cannot be reduced). The unpopulated $4000-$7FFF range therefore reads back written values instead of returning 0xFF. A `TODO` entry has been added documenting that the system YAML needs a mechanism to insert custom read/write guards (e.g. an `unpopulated` region type or per-region `read_guard`/`write_guard` snippets) so $4000-$7FFF returns 0xFF on read and ignores writes — this fix is deferred per the system-YAML-driven design principle.
