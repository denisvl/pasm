...
"""
    ...
    if subsystem_builds:
        subsystem_add_subdirs = "\n".join(
            f'add_subdirectory("{_cmake_path(str(item["cmake_subdir"]))}")'
            for item in subsystem_builds
            if str(item.get("cmake_subdir", "")).strip()
        )
        link_targets: list[str] = []
        for item in subsystem_builds:
            subsystem_cpu_core_target = str(item.get("cpu_core_target", "")).strip()
            system_target_name = str(item.get("system_target", "")).strip()
            if subsystem_cpu_core_target:
                link_targets.append(subsystem_cpu_core_target)
            if system_target_name:
                link_targets.append(system_target_name)
        if link_targets:
            subsystem_system_link_block = (
                f"target_link_libraries({system_target} PUBLIC\n"
                + "\n".join(f"    {target}" for target in link_targets)
                + "\n)\n"
            )
            subsystem_test_link_block = (
                f"target_link_libraries({project_name}_test PRIVATE\n"
...
"""
    ...
    return template.format(
        ...
        subsystem_add_subdirs=subsystem_add_subdirs,
        subsystem_system_link_block=subsystem_system_link_block,
        subsystem_test_link_block=subsystem_test_link_block,
...
def generate_makefile(
    ...
    subsystem_static_libs: list[str] = []
    for item in subsystem_builds:
        subdir = _make_path(str(item.get("cmake_subdir", "")))
        system_static = str(item.get("system_static", "")).strip()
        cpu_core_static = str(item.get("cpu_core_static", "")).strip()
        if subdir and system_static:
            subsystem_static_libs.append(f"{subdir}/{system_static}")
        if subdir and cpu_core_static:
            subsystem_static_libs.append(f"{subdir}/{cpu_core_static}")
    subsystem_prereqs = " ".join(subsystem_static_libs)
    subsystem_link_inputs = (" " + " ".join(subsystem_static_libs)) if subsystem_static_libs else ""
    subsystem_all_rule = ""
    subsystem_clean_rule = ""
    if subsystem_dirs:
        subsystem_all_rule = "\n".join(
            f"\t$(MAKE) -C {subdir} all" for subdir in subsystem_dirs
        )
        subsystem_clean_rule = "\n".join(
            f"\t$(MAKE) -C {subdir} clean" for subdir in subsystem_dirs
        )
    all_prereqs = "$(TARGET)"
    target_prereqs = "$(MAIN_OBJ) $(SYSTEM_LIB) $(CPU_CORE_LIB)"
    if subsystem_prereqs:
        all_prereqs = f"{subsystem_prereqs} $(TARGET)"
        target_prereqs = f"{target_prereqs} {subsystem_prereqs}"
...
all: {all_prereqs}
{subsystem_all_rule if subsystem_all_rule else ""}

$(TARGET): {target_prereqs}
\t$(CC) $(LDFLAGS) -o $@ $(MAIN_OBJ) $(CPU_CORE_LIB) $(SYSTEM_LIB){subsystem_link_inputs} $(CPU_CORE_LIB)
...
clean:
{subsystem_clean_rule if subsystem_clean_rule else ""}\n\trm -rf $(BUILD_DIR) $(TARGET) $(CPU_CORE_LIB) $(SYSTEM_LIB)
"""
